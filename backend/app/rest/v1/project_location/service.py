...
from .model import ProjectLocation
from .resource import ProjectLocationList
from .schema import ProjectLocationSchema
...
class ProjectLocationService(BaseService):
    def __init__(self, **kwargs):
        ...
        self.schema = ProjectLocationSchema
        self.resources = [
            {'resource': ProjectLocationList, 'url': '/project-locations'}
        ]
    ...
    def manage_project_locations(self, records, project_detail_id, delete=False):
        if delete:
            ids = []
...
def update_project_location(self, project_detail, **kwargs):
    records = kwargs.get("locations", None)
    if records is not None:
        service = ProjectLocationService()
        service.manage_project_locations(
