...
schema["parent"] = fields.Nested(
    "FunctionSchema", exclude=("children",), dump_only=True)
schema["children"] = fields.Nested(
    "FunctionSchema", exclude=("parent",), many=True, dump_only=True)
schema["level"] = fields.Integer(dump_only=True)
schema["additional_data"] = fields.Raw(required=False, allow_none=True)

schema = generate_schema("function", schema)
FunctionSchema = type("FunctionSchema", (BaseSchema,), schema)
