...
from .model import OmCost
from .resource import OmCostList
from .schema import OmCostSchema


class OmCostService(BaseService):
    def __init__(self, **kwargs):
        super().__init__()
        self.model = OmCost
        self.schema = OmCostSchema
        self.resources = [
            {'resource': OmCostList, 'url': '/om-costs'}
        ]
    ...
    def manage_om_costs(self, records, project_detail_id, delete=False):
        if delete:
            ids = []
        ...
        for record in records:
            if 'fund_id' in record and 'costing_id' in record:
...
def update_om_costs(self, project_detail, **kwargs):
    records = kwargs.get("om_costs", None)
    if records is not None:
        service = OmCostService()
        service.manage_om_costs(
