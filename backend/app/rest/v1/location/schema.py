...
schema["parent"] = fields.Nested(
    "LocationSchema", exclude=("children",), dump_only=True)
schema["children"] = fields.Nested(
    "LocationSchema", exclude=("parent",), many=True, dump_only=True)
schema["level"] = fields.Integer(dump_only=True)
schema["additional_data"] = fields.Raw(required=False, allow_none=True)

schema = generate_schema("location", schema)
LocationSchema = type("LocationSchema", (BaseSchema,), schema)
