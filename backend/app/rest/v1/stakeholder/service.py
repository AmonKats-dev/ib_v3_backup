...
from .resource import StakeholderList
from .schema import StakeholderSchema


class StakeholderService(BaseService):
    def __init__(self, **kwargs):
        super().__init__()
        self.model = Stakeholder
        self.schema = StakeholderSchema
        self.resources = [
            {'resource': StakeholderList, 'url': '/stakeholders'}
        ]
    ...
    def manage_stakeholders(self, records, project_detail_id, delete=False):
        if delete:
            ids = []
...
def update_stakeholders(self, project_detail, **kwargs):
    records = kwargs.get("stakeholders", None)
    if records is not None:
        service = StakeholderService()
        service.manage_stakeholders(
