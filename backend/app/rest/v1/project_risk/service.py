...
from .model import ProjectRisk
from .resource import ProjectRiskList
from .schema import ProjectRiskSchema


class ProjectRiskService(BaseService):
    def __init__(self, **kwargs):
        super().__init__()
        self.model = ProjectRisk
        self.schema = ProjectRiskSchema
        self.resources = [
            {'resource': ProjectRiskList, 'url': '/project-risks'}
        ]
    ...
    def manage_project_risks(self, records, project_detail_id, delete=False):
        if delete:
            ids = []
...
def update_project_risk(self, project_detail, **kwargs):
    records = kwargs.get("project_risks", None)
    if records is not None:
        service = ProjectRiskService()
        service.manage_project_risks(
