...
from .model import ClimateRisk
from .resource import ClimateRiskList
from .schema import ClimateRiskSchema


class ClimateRiskService(BaseService):
    def __init__(self, **kwargs):
        super().__init__()
        self.model = ClimateRisk
        self.schema = ClimateRiskSchema
        self.resources = [
            {'resource': ClimateRiskList, 'url': '/climate-risks'}
        ]
    ...
    def manage_climate_risks(self, records, project_detail_id, delete=False):
        if delete:
            ids = []
...
def update_climate_risk(self, project_detail, **kwargs):
    records = kwargs.get("climate_risks", None)
    if records is not None:
        service = ClimateRiskService()
        service.manage_climate_risks(
