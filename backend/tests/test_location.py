...
COMPONENT_URL = f'{API_V1_PREFIX}/locations'


def test_location_empty(client):
...
def test_location_crud(client):
    token = get_valid_token(client)
    location_create(client, token)
    location_list(client, token)
    location_read(client, token)
    location_update(client, token)
    location_delete(client, token)
...
def test_location_filter(client):
    ...
    data = json.loads(response.data)
    assert len(data) == 1
    location_create(client, token, 1, 2)
    response = client.get(
...
def location_list_empty(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def location_list(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def location_read(client, token, id=1, parent_id=None):
    response = client.get(
        f'{COMPONENT_URL}/{id}',
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def location_create(client, token, parent_id=None, level=1):
    data = {
        "code": "001",
...
def location_update(client, token):
    data = dict()
...
def test_location_without_code(client):
    token = get_valid_token(client)
    data = {
...
def test_location_multi_level(client):
    token = get_valid_token(client)
    location_create(client, token)
    location_create(client, token, 1, 2)
    location_read(client, token, id=2, parent_id=1)


def test_location_wrong_multi_level(client):
    token = get_valid_token(client)
    location_create(client, token)
    data = {
        "code": "001",
...
def test_location_delete_parent(client):
    token = get_valid_token(client)
    location_create(client, token)
    location_create(client, token, 1, 2)
    location_delete(client, token)
...
def location_delete(client, token):
