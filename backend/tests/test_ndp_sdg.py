...
def test_ndp_sdg_crud(client):
    token = get_valid_token(client)
    ndp_sdg_create(client, token)
    ndp_sdg_list(client, token)
    ndp_sdg_read(client, token)
    ndp_sdg_update(client, token)
    ndp_sdg_delete(client, token)


def test_ndp_sdg_filter(client):
    token = get_valid_token(client)
...
def ndp_sdg_list_empty(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def ndp_sdg_create(client, token):
    data = {
        'name': 'Strategy 1',
        'ndp_outcome_id': 1
    }
...
def ndp_sdg_update(client, token):
    data = dict()
...
def ndp_sdg_delete(client, token):
