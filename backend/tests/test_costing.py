...
COMPONENT_URL = f'{API_V1_PREFIX}/costings'


def test_costing_empty(client):
...
def test_costing_crud(client):
    token = get_valid_token(client)
    costing_create(client, token)
    costing_list(client, token)
    costing_read(client, token)
    costing_update(client, token)
    costing_delete(client, token)


def test_costing_filter(client):
    token = get_valid_token(client)
    costing_create(client, token)
    response = client.get(
    ...
    data = json.loads(response.data)
    assert len(data) == 1
    costing_create(client, token, 1, 2)
    response = client.get(
...
def costing_list_empty(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def costing_read(client, token, id=1, parent_id=None):
    response = client.get(
        f'{COMPONENT_URL}/{id}',
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def costing_create(client, token, parent_id=None, level=1):
    data = {
        "code": "001",
...
def costing_update(client, token):
    data = dict()
...
def test_costing_without_code(client):
    token = get_valid_token(client)
    data = {
...
def test_costing_multi_level(client):
    token = get_valid_token(client)
    costing_create(client, token)
    costing_create(client, token, 1, 2)
    costing_read(client, token, id=2, parent_id=1)


def test_costing_wrong_multi_level(client):
    token = get_valid_token(client)
    costing_create(client, token)
    data = {
        "code": "001",
...
def test_costing_delete_parent(client):
    token = get_valid_token(client)
    costing_create(client, token)
    costing_create(client, token, 1, 2)
    costing_delete(client, token)
...
def costing_delete(client, token):
