...
COMPONENT_URL = f'{API_V1_PREFIX}/functions'


def test_function_empty(client):
...
def test_function_crud(client):
    token = get_valid_token(client)
    function_create(client, token)
    function_list(client, token)
    function_read(client, token)
    function_update(client, token)
    function_delete(client, token)


def test_function_filter(client):
    token = get_valid_token(client)
    function_create(client, token)
    response = client.get(
    ...
    data = json.loads(response.data)
    assert len(data) == 1
    function_create(client, token, 1, 2)
    response = client.get(
...
def function_list_empty(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def function_list(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def function_read(client, token, id=1, parent_id=None):
    response = client.get(
        f'{COMPONENT_URL}/{id}',
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def function_create(client, token, parent_id=None, level=1):
    data = {
        "code": "001",
...
def function_update(client, token):
    data = dict()
...
def test_function_without_code(client):
    token = get_valid_token(client)
    data = {
...
def test_function_multi_level(client):
    token = get_valid_token(client)
    function_create(client, token)
    function_create(client, token, 1, 2)
    function_read(client, token, id=2, parent_id=1)


def test_function_wrong_multi_level(client):
    token = get_valid_token(client)
    function_create(client, token)
    data = {
        "code": "001",
...
def test_function_delete_parent(client):
    token = get_valid_token(client)
    function_create(client, token)
    function_create(client, token, 1, 2)
    function_delete(client, token)
...
def function_delete(client, token):
