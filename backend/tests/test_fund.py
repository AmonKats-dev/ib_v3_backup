...
COMPONENT_URL = f'{API_V1_PREFIX}/funds'


def test_fund_empty(client):
...
def test_fund_crud(client):
    token = get_valid_token(client)
    fund_create(client, token)
    fund_list(client, token)
    fund_read(client, token)
    fund_update(client, token)
    fund_delete(client, token)


def test_fund_filter(client):
    token = get_valid_token(client)
    fund_create(client, token)
    response = client.get(
...
def fund_list_empty(client, token):
    response = client.get(
        COMPONENT_URL,
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def fund_read(client, token, id=1, parent_id=None):
    response = client.get(
        f'{COMPONENT_URL}/{id}',
        headers=dict(Authorization="Bearer " + token),
        content_type="application/json",
...
def fund_create(client, token, parent_id=None, level=1):
    data = {
        "code": "001",
...
def test_fund_without_code(client):
    token = get_valid_token(client)
    data = {
...
def test_fund_multi_level(client):
    token = get_valid_token(client)
    fund_create(client, token)
    fund_create(client, token, 1, 2)
    fund_read(client, token, id=2, parent_id=1)


def test_fund_wrong_multi_level(client):
    token = get_valid_token(client)
    fund_create(client, token)
    data = {
        "code": "001",
...
def test_fund_delete_parent(client):
    token = get_valid_token(client)
    fund_create(client, token)
    fund_create(client, token, 1, 2)
    fund_delete(client, token)
